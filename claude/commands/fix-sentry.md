...
4. **Écrire des specs de non-régression** (OBLIGATOIRE) :
   - Trouver le fichier de spec existant correspondant au code modifié
   - Si aucun fichier de spec n'existe, en créer un
   - Écrire au minimum :
     - Un test reproduisant le scénario qui causait l'erreur (le cas qui crashait)
     - Un test vérifiant le comportement normal (le happy path n'est pas cassé)
   - Lancer les specs localement si possible (`bundle exec rspec <fichier>`)
   - Lancer rubocop sur les fichiers modifiés
   - Corriger les éventuels échecs avant de continuer
5. `git add <fichiers>` puis commit :
   ```
...
7. Créer la PR draft avec `gh pr create --draft` :
   ...
   - Body avec ce template :
     ...
     ```
8. Vérifier que `git branch --show-current` correspond bien à la branche attendue avant chaque commit (protection contre les switchs de branche parasites)
...
- **Toujours écrire des specs de non-régression** — pas de PR sans spec
