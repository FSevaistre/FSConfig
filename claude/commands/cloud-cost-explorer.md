...
## Etape prealable — Charger la configuration cloud

Lis le fichier `~/.claude/cloud-accounts.json` pour obtenir les identifiants des comptes cloud. Format attendu :
```json
{
  "gcp_project": "...",
  "gcp_billing_account": "...",
  "aws_profiles": ["admin-profile", "prod-profile", "staging-profile", "shared-profile"],
  "aws_default_profile": "admin-profile"
}
```
Utilise ces valeurs partout ou un nom de projet GCP, un billing account ID, ou un profil AWS est necessaire.
...
- **"all"** : rapport sur tous les comptes AWS (tous les profils de aws_profiles) + GCP
- **"account <profil>"** : rapport sur un compte AWS specifique (ex: "account prod-profile")
...
- Par defaut : la valeur `aws_default_profile` du fichier cloud-accounts.json
- Si l'argument contient "account <profil>" : utiliser ce profil
- Si l'argument contient "all" : iterer sur tous les profils de `aws_profiles`
...
```
Si ca echoue, utiliser le FALLBACK GMAIL avant de demander un login :

### Fallback Gmail (quand SSO expire)

Chercher le dernier email "AWS Budgets Report: daily" via `mcp__claude_ai_Gmail__gmail_search_messages` :
- q = `from:budgets subject:"AWS Budgets Report" newer_than:2d`
- maxResults = 1

Lire le contenu avec `mcp__claude_ai_Gmail__gmail_read_message`. Le rapport contient :
- Budget Name, Budget Type, Current spend, Budgeted amount, Forecasted spend
- Ces donnees suffisent pour le resume (total mois en cours + prevision)

Si le fallback fonctionne, utiliser ces donnees pour la section AWS du rapport et mentionner "(source: AWS Budgets email, SSO expire)" dans le rapport. NE PAS bloquer le rapport complet — continuer avec GCP normalement.

Si le fallback echoue aussi, dire a l'utilisateur de lancer `! aws sso login --profile <profil>`.
...
- Projet principal : valeur `gcp_project` de cloud-accounts.json
- Region BigQuery : `region-eu`
- Billing account ID : valeur `gcp_billing_account` de cloud-accounts.json (format XX-XX-XX)
...
L'export billing est configure dans `<GCP_PROJECT>.billing_export`. Le nom de la table se derive du billing account ID : `gcp_billing_export_v1_<BILLING_ID_UNDERSCORES>` (remplacer les tirets par des underscores dans l'ID).

```bash
bq query --use_legacy_sql=false --project_id=<GCP_PROJECT> --format=csv --max_rows=500 "
SELECT
...
FROM \`<GCP_PROJECT>.billing_export.gcp_billing_export_v1_<BILLING_ID_UNDERSCORES>\`
...
```bash
bq query --use_legacy_sql=false --project_id=<GCP_PROJECT> --format=csv --max_rows=200 "
SELECT
...
- URL : https://console.cloud.google.com/billing/<GCP_BILLING_ACCOUNT>/reports/tabular
