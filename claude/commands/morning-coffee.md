...
IMPORTANT — ATTENDRE les resultats Lucca avant de passer a l'etape 2. Les absences sont necessaires pour filtrer les 1:1 inutiles. L'etape 1ter (wrap-up) peut commencer en parallele car elle concerne les 1:1 d'HIER, pas d'aujourd'hui.
...
### Filtre absences Lucca

AVANT de lancer un /1to1, verifier si le managee est absent aujourd'hui d'apres les donnees Lucca (etape 1bis). Si le managee est en conge/absent :
- NE PAS lancer le /1to1 (economie de temps et de context)
- Noter dans le briefing : "1:1 [Prenom] ([heure]) — ANNULE (conge jusqu'au [date retour])"
- Decompter la duree du meeting des temps en reunion (le 1:1 n'aura pas lieu)
- Ne PAS compter ce 1:1 dans le nombre de 1:1 pour la logique sequentiel/parallele ci-dessous
