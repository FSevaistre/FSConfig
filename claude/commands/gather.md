...
### Expiration des items OUVERT

Quand tu reprends les items OUVERT du gather precedent (section SUIVI), applique cette regle d'expiration :

- Si un item est marque "TOUJOURS OUVERT" depuis 3 gathers consecutifs ou plus SANS aucun mouvement (pas de changement de statut, pas de nouveau signal dans les donnees collectees), le deplacer dans une section **GELE** a la fin du gather, avec la mention "(gele apres 3 gathers sans mouvement)".
- Les items GELE ne sont PAS repris dans le SUIVI du gather suivant. Ils disparaissent du cycle.
- Si un item gele reapparait naturellement dans les donnees (mention Slack, email, PR, etc.), il est automatiquement reactive et remis dans OUVERT.
- Exception : les items tagges avec le mot "strategique" ou "bloquant" dans leur description ne sont JAMAIS geles, meme apres 3+ gathers sans mouvement.

Cela evite que la section OUVERT grossisse indefiniment avec des items morts.
