...
```bash
# 1. Bootstrap minimal (git + GitHub CLI)
sudo apt install -y git curl

# 2. 1Password (fondation — gere SSH, secrets, mots de passe)
curl -sS https://downloads.1password.com/linux/keys/1password.asc | sudo gpg --dearmor --output /usr/share/keyrings/1password-archive-keyring.gpg
echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/1password-archive-keyring.gpg] https://downloads.1password.com/linux/debian/$(dpkg --print-architecture) stable main" | sudo tee /etc/apt/sources.list.d/1password.list
sudo apt update && sudo apt install -y 1password 1password-cli
# -> Ouvrir 1Password, se connecter, activer SSH agent + CLI integration dans Settings > Developer
...
# 5. Clone et install
git clone https://github.com/FSevaistre/FSConfig.git ~/FSConfig
~/FSConfig/install.sh       # interactif
# ou
~/FSConfig/install.sh --all # tout d'un coup
```
...
│   ├── team.json           # Arborescence equipe (PLACEHOLDER — le vrai vit dans ~/.claude/)
│   ├── commands/           # Slash commands (/1to1, /gather, /morning-coffee, /lucca, etc.)
│   └── skills/             # Skills auto-detectes (/pr, /transcribe, /company-info)
├── scripts/
│   └── gdrive-dl          # Script Python : download/search Google Drive (OAuth2)
├── keyboard/
│   └── setup.sh           # Caps Lock -> Escape
├── requirements.txt        # Dependances Python (gdrive-dl, whisper)
...
# Claude Code
mkdir -p ~/.claude ~/.claude/commands ~/.claude/skills ~/.local/bin
cp FSConfig/claude/settings.json ~/.claude/settings.json
cp FSConfig/claude/commands/*.md ~/.claude/commands/
cp -r FSConfig/claude/skills/* ~/.claude/skills/
cp FSConfig/scripts/* ~/.local/bin/ && chmod +x ~/.local/bin/*
pip3 install --user -r FSConfig/requirements.txt
...
`/market-watch` -- Veille marche du credit immobilier. Recherche les signaux recents (taux BCE, OAT 10 ans, politique bancaire, HCSF, prix immobilier, concurrence courtiers) et produit un resume structure avec signaux forts/faibles et impact Pretto.

`/cloud-cost-explorer [aws|gcp|detail <service>|compare|all]` -- Rapport de couts cloud AWS + GCP. Vue CTO : total mois, prevision, top services, cout journalier, detection d'anomalies. Fallback Gmail quand AWS SSO expire. GCP via BigQuery billing export.

`/collect-transcripts` -- Collecte 20 transcripts d'appels Salesforce au hasard et les met dans Notion pour analyse qualite.

`/week-wrap-up` -- Bilan hebdomadaire CTO : decisions, people, code, plan d'action, temps. Agregat de la semaine.
